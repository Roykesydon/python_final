...
class myGUI:
    ...
    def getReward(self, rewardNumber, targetNum, winTicket):
        if len(targetNum) == 0:
            return 0
        # print('hey',targetNum[-8:].replace('\n','').replace(' ',''),rewardNumber[0].replace('\n','').replace(' ',''),targetNum[-8:].replace('\n','').replace(' ','')==rewardNumber[0].replace('\n','').replace(' ',''),type(targetNum[-8:]),type(rewardNumber[0]))
        if targetNum[-8:].replace('\n', '').replace(' ', '') == rewardNumber[0].replace('\n', '').replace(' ', ''):
            winTicket.append(
                targetNum+'/'+rewardNumber[4]+'/'+rewardNumber[5]+':10000000')
            return 10000000
        if targetNum[-8:].replace('\n', '').replace(' ', '') == rewardNumber[1].replace('\n', '').replace(' ', ''):
            winTicket.append(
                targetNum+'/'+rewardNumber[4]+'/'+rewardNumber[5]+':2000000')
            return 2000000
        for i in rewardNumber[2]:
            i = i.replace('\n', '').replace(' ', '')
            if i == targetNum[-8:].replace('\n', '').replace(' ', ''):
                winTicket.append(
    ...
    def cal(self, start, end):
        ...
        global dict1000, dict200, city1000, city200
        dict1000 = collections.Counter()
        dict200 = collections.Counter()
        city1000 = collections.Counter()
        city200 = collections.Counter()
    ...
    def __init__(self):
        ...
        self.label = tk.Label(
            self.master, text='輸入發票號碼到同資料夾下的\'PYTHON發票號碼測資.csv\'')
        self.label.pack()
    ...
    def autoChecking(self):
        ...
        with open("./PYTHON發票號碼測資.csv", 'r', encoding='utf-8', newline='') as csvfile:
